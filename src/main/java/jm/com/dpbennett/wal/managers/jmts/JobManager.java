 ...
 */
package jm.com.dpbennett.wal.managers.jmts;
...
import jm.com.dpbennett.wal.JMTSApplication;
...
//        getPurchasingManager().openPurchaseReqsTab();
    ...
    private void initMainTabView() {
     ...
     private void initDashboard() {
         if (getUser().getModules().getJobManagementAndTrackingModule()) {
            getSystemManager().addDashboardTab(new TabPanel("Job Management",
                    "Job Management"));
        }        
        if (getUser().getModules().getLegalOfficeModule()) {
            getSystemManager().addDashboardTab(new TabPanel("Document Management", "Document Management"));
        }
        if (getUser().getModules().getCrmModule()) {
            getSystemManager().addDashboardTab(new TabPanel("Client Management",
                    "Client Management"));
        }
        if (getUser().getModules().getAdminModule()) {
        ...
        if (getUser().getModules().getFinancialAdminModule()) {
            getSystemManager().addDashboardTab(new TabPanel("Financial Administration", "Financial Administration"));
        }
        if (getUser().getModules().getPurchaseManagementModule()) { 
            getSystemManager().addDashboardTab(new TabPanel("Procurement", "Procurement"));
        }
